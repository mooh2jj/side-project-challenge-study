...
리두 로그는 MySQL 서버가 비정상적으로 종료되었을 때 데이터 파일에 기록되지 못한 데이터를 잃지 않게 해주는 안전장치 역할을 하며 종료 되었을 때 아래와 같은 데이터를 가질 수 있다.

1. 커밋됐지만 데이터 파일에 기록되지 않은 데이터
2. 롤백됐지만 데이터 파일에 이미 기록된 데이터

1번의 경우 리두 로그에 저장된 데이터를 복사하면 되지만 2번의 경우 언두 로그의 내용을 가져와 복사해야 하며 이 과정에서 트랜잭션(커밋, 롤백, 실행 중)을 확인하기 위해 리두 로그도 필요하다.
...
![image](https://user-images.githubusercontent.com/43610417/227687968-71d949c1-f8e7-4e0c-9544-35f245c88256.png)
...
버퍼 풀의 더티 페이지 비율과 리두 로그 파일의 전체 크기는 쓰기 버퍼링 기능 성능에 관계되어 있기에 DB의 최적화를 위해서는 전체 크기의 최적 값을 찾아야 한다.

버퍼 풀에 쿼리들이 사용한 데이터가 올라가 있기에 디스크에서 데이터를 읽지 않아도 되기에 쿼리 성능이 향상된다. 

그리고 쿼리 요청이 빈번한 서버의 경우 재시작시 성능이 저하되는 데 그 이유는 버퍼 풀에 데이터가 없기에 그렇다.

MySQL5.5에서는 풀 스캔을 실행하고 서비스를 오픈했지만 5.6부터는 버퍼 풀 덤프  적재 기능으로 innodb_buffer_pool_dump_now 시스템 변수로 버퍼 풀을 백업할 수 있다.

### Double Write Buffer

리두 로그는 공간의 낭비를 막기 위해 변경된 내용만 기록한다. 그렇지만 비정상 종료될 때 더티 페이지를 디스크 파일로 플러시 할 때 일부만 기록되는 문제시 복구를 할 수 없기에 **Double Write Buffer** 기법을 활용해 문제를 해결한다.

![image](https://user-images.githubusercontent.com/43610417/227688861-0b4d64e4-2ad9-422c-ab30-3cc56fed63e7.png)
...
**언두 로그란** INSERT, UPDATE, DELETE로 변경되기 이전 데이터를 백업해둔 데이터를 말한다.
...
언두 로그가 저장되는 공간인 언두 테이블 스페이스는 서버가 초기화 될 때 생성되기에 확장에 한계가 있다.
